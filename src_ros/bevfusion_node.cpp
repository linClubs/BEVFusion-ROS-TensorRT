...
std::shared_ptr<BEVFusionNode> bevfusion_node;
  ...
  const sensor_msgs::ImageConstPtr& msg_br_img)
{
  ...
  bevfusion_node->Inference(images, lidar_arr, cloud_ptr->points.size());

  cv::Mat img = cv::imread((pkg_path + "/build/cuda-bevfusion.jpg").c_str());
...
{  
  ...
  bevfusion_node = std::make_shared<BEVFusionNode>();
  ...
  sub_br_img.subscribe(n,"/cam_back_right/raw", 10);
	...
	auto sync = std::make_shared<Sync>( MySyncPolicy(10), sub_cloud, 
    sub_f_img,  sub_fl_img, sub_fr_img,
    sub_b_img ,sub_bl_img, sub_br_img); 
	
  sync->registerCallback(boost::bind(&callback, _1, _2,_3, _4, _5, _6,_7)); // 绑定回调函数
